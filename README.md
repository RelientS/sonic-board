...
| --- | --- | --- | --- | --- |
| MXR Dyna Comp | PedalKernel WDF / WASM | 持续输出、有限值和控制响应通过 | 未完成 | 电路候选，目标 ≥8，未评分 |
| Pro Co RAT 2 | PedalKernel WDF / WASM | 持续输出、有限值和控制响应通过 | 未完成 | 电路候选，目标 ≥8，未评分 |
| Boss BD-2 Blues Driver | Legacy Web Audio | PedalKernel 候选未通过持续输出门禁 | 未完成 | 暂不启用 PedalKernel |
| Electro-Harmonix Big Muff Pi | Legacy Web Audio | PedalKernel 候选未通过持续输出门禁 | 未完成 | 暂不启用 PedalKernel |

其余效果器、箱头与箱体目前仍是非官方算法近似。经典名称只用于说明参考对象，不表示厂商授权或官方模型。

PedalKernel 固定在提交 `0278b397c861b5ebef2e8e38d15ab281b8e669dc`。浏览器运行层位于 `dsp/pedalkernel-wasm`，预编译产物为 `public/audio/pedalkernel.wasm`，实时处理器为 `public/audio/pedalkernel-processor.js`。
